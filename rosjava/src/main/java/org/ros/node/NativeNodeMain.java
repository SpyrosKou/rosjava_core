...
public abstract class NativeNodeMain extends AbstractNodeMain {
  ...
  public NativeNodeMain(String libName, String[] remappings) {
    ...
    logger.info("Trying to load native library '" + libName + "'...");
    try {
      System.loadLibrary(libName);
    }
    catch (SecurityException e) {
      logger.info("Error loading library! SecurityException");
    }
    catch (UnsatisfiedLinkError e) {
      logger.info("Error loading library! UnsatisfiedLinkError");
    }
    catch (NullPointerException e) {
      logger.info("Error loading library! NullPointerException");
    }
  }
