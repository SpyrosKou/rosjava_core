...
public final class MasterRegistrationManagerImpl {
     ...
     *
     * @param nodeName         name of the node with the publisher
     * @param nodeSlaveUri     URI of the slave server on the node
     * @param topicName        then name of the topic
     ...
     *
     * @param nodeName  name of the node which has the publisher
     * @param topicName name of the publisher's topic
        ...
        if (topic != null) {
            ...
            if (node != null) {
                ...
                if (LOGGER.isWarnEnabled()) {
                    LOGGER.warn(String.format("Received unregister publisher for topic %s on unknown node %s",
                            topicName, nodeName));
                }
            ...
            if (LOGGER.isWarnEnabled()) {
                LOGGER.warn(String.format("Received unregister publisher for unknown topic %s on node %s",
                        topicName, nodeName));
            }
     ...
     *
     * @param nodeName         name of the node with the subscriber
     * @param nodeSlaveUri     URI of the slave server on the node
     * @param topicName        then name of the topic
     * @param topicMessageType message type of the topic
     * @return The registration information for the topic.
     ...
     *
     * @param nodeName  name of the node which has the subscriber
     * @param topicName name of the subscriber's topic
    ...
    public boolean unregisterSubscriber(GraphName nodeName, GraphName topicName) {
        ...
        if (topic != null) {
            ...
            if (node != null) {
                ...
                // never was a node with that name
                if (LOGGER.isWarnEnabled()) {
                    LOGGER.warn(String.format("Received unregister subscriber for topic %s on unknown node %s",
                            topicName, nodeName));
                }
                return false;
            }
            ...
            // If no topic, there will be no node registration.
            if (LOGGER.isWarnEnabled()) {
                LOGGER.warn(String.format("Received unregister subscriber for unknown topic %s on node %s",
                        topicName, nodeName));
            }
            return false;
        }
     ...
     * Register a service.
     *
     * @param nodeName     name of the node with the service
     * @param nodeSlaveUri URI of the slave server on the node
    ...
    public boolean unregisterService(GraphName nodeName, GraphName serviceName, URI serviceUri) {
        ...
        if (service != null) {
            ...
            if (node != null) {
                ...
                // never was a node with that name
                if (LOGGER.isWarnEnabled()) {
                    LOGGER.warn(String.format("Received unregister for service %s on unknown node %s",
            ...
            if (LOGGER.isWarnEnabled()) {
                LOGGER.warn(String.format("Received unregister for unknown service %s on node %s",
                        serviceName, nodeName));
            }
     ...
     *
     * @param topicName    the name of the topic
     * @param shouldCreate {@code true} if a new one should be created if it isn't found
     ...
     *
     * @param nodeName     the name of the node
     * @param nodeSlaveUri the URI for the node's slave server
