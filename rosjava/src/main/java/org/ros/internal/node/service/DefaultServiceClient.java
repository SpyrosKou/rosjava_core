...
final class DefaultServiceClient<T extends Message, S extends Message> implements ServiceClient<T, S> {
    ...
    /**
     * {@link RosRuntimeException} will be thrown if the connection is not established
     *
     * @param uri
     * @return true if connection was just established, false if it was already connected
     */
    public final synchronized void connectIfUnconnected(final URI uri) {
        if (!this.isConnected()) {
            this.connect(uri);
        }
    }
