...
public class DefaultPublisher<T extends Message> extends DefaultTopicParticipant implements Publisher<T> {
    ...
    public void shutdown(long timeout, TimeUnit unit) {
        this.shutdownLatch = new CountDownLatch(listeners.size());
    ...
    public final ChannelBuffer finishHandshake(final ConnectionHeader incomingHeader) {
        ...
        if (DEBUG ) {
            LOGGER.debug("Subscriber handshake header: " + incomingHeader);
            LOGGER.debug("Publisher handshake header: " + topicDefinitionHeader);
        }
        ...
        final boolean messageTypeMatches = incomingType.equals(expectedType) || incomingType.equals(Subscriber.TOPIC_MESSAGE_TYPE_WILDCARD);
     ...
     *
     * @param subscriberIdentifer the {@link SubscriberIdentifier} of the new subscriber
     * @param channel             the communication {@link Channel} to the {@link Subscriber}
     */
    public final void addSubscriber(SubscriberIdentifier subscriberIdentifer, Channel channel) {
        if (DEBUG) {
            LOGGER.debug(String.format("Adding subscriber %s channel %s to publisher %s.",
    ...
    @Override
    public final void addListener(final PublisherListener<T> listener) {
     ...
     */
    @Override
    public boolean removeListener(final PublisherListener<T> listener) {
    ...
    public final void signalOnMasterRegistrationSuccess() {
        this.listeners.signal(listener -> listener.onMasterRegistrationSuccess(this));
    }
    ...
    public final void signalOnMasterRegistrationFailure() {
        ...
        listeners.signal(listener -> listener.onMasterRegistrationFailure(publisher));
    }
     ...
     * Each listener is called in a separate thread.
     *
     * @param subscriberIdentifier the {@link SubscriberIdentifier} of the new {@link Subscriber}
     */
    private final void signalOnNewSubscriber(final SubscriberIdentifier subscriberIdentifier) {
        final Publisher<T> publisher = this;
        listeners.signal(listener -> listener.onNewSubscriber(publisher, subscriberIdentifier));
    }
    ...
    private final void signalOnShutdown(long timeout, TimeUnit unit) {
        final Publisher<T> publisher = this;
        try {
            listeners.signal(listener -> listener.onShutdown(publisher), timeout, unit);
        } catch (InterruptedException e) {
