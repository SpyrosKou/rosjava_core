...
public final class ConnectionHeader {
    ...
    public static ConnectionHeader decode(ChannelBuffer buffer) {
        ...
        while (position < readableBytes) {
            final int fieldSize = buffer.readInt();
            position += 4;
            if (fieldSize == 0) {
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Decoded header: " + fields);
        }
     ...
     * transmission
     */
    public final ChannelBuffer encode() {
        final ChannelBuffer buffer = MessageBuffers.dynamicBuffer();
        for (final Entry<String, String> entry : this.fields.entrySet()) {
            final String field = entry.getKey() + "=" + entry.getValue();
    ...
    public final void merge(final ConnectionHeader other) {
        final Map<String, String> otherFields = other.getFields();
        this.mergeFields(otherFields);
    }

    public final void mergeFields(final Map<String, String> other) {
        for (final Entry<String, String> field : other.entrySet()) {
