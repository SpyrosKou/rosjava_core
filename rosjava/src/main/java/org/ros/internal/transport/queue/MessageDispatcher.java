...
public final class MessageDispatcher<T extends Message> extends CancellableLoop {
    ...
    /**
     * @param enabled {@code true} if latch mode should be enabled, {@code false}
     *                otherwise
     */
    ...
    public final void loop() throws InterruptedException {
        final LazyMessage<T> lazyMessage = this.lazyMessages.takeFirst();
        synchronized (this.mutex) {
            this.latchedMessage = lazyMessage;
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("Dispatching message: " + latchedMessage.get());
            }
            this.messageListeners.signal(newSignaler(latchedMessage));
        }
    }
