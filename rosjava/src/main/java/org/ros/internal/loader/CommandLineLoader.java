...
public final class CommandLineLoader {
    ...
    private String getHost() {
        ...
        if (this.specialRemappings.containsKey(CommandLineVariables.ROS_IP)) {
            host = this.specialRemappings.get(CommandLineVariables.ROS_IP);
        } else if (this.environment.containsKey(EnvironmentVariables.ROS_IP)) {
            host = this.environment.get(EnvironmentVariables.ROS_IP);
        } else if (this.environment.containsKey(EnvironmentVariables.ROS_HOSTNAME)) {
            host = this.environment.get(EnvironmentVariables.ROS_HOSTNAME);
        } else {
            host = InetAddressFactory.newLoopback().getHostAddress();
        }
